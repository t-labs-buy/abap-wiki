...
- 2026-07-21: Adopted CP-3 (mandatory applicability scoping) and CP-4 (conflict detection at ingest) from PROPOSAL - Organizational Memory Adoption. CLAUDE.md: Quality Bar test 5 (explicit scope on Zone 03 pages) and new ingestion step C2 Conflict Check (consistent/refines/contradicts triage, conflict callout convention, curator-owned resolution). Templates: Scope sections added to _Template-Gotcha, _Template-Pattern, _Template-Troubleshooting. Migrated [[Gotcha - BAPI_TRANSACTION_COMMIT wait flag]] and [[Pattern - BAPI_TRANSACTION_COMMIT WAIT in batch jobs]] with Applies to / Does not apply to. Ingest pipeline prompt: new rules 10b (scope) and 17 (conflict check). conventions.md updated.

- 2026-07-22: Adopted compile-time backlinks (from LLM knowledge-base tutorial, louiswang524.github.io/blog/llm-knowledge-base). CLAUDE.md: new "Backlinks — ## Linked from" subsection under Linking Rules + backlink-maintenance step in the ingestion workflow. Ingest pipeline prompt: new rule 4b (bidirectional link maintenance). conventions.md: Backlinks section. Backfilled `## Linked from` sections on all 30 content pages (entries as `- [[Source]] (type)`, alphabetical) and added stubs to all 23 templates. Frontmatter `updated:` dates left untouched — mechanical link maintenance, no content change. Side finding: fixed during backfill analysis — plain [[OTC]] resolves to the workstream page per vault convention; the Open-Questions page is always linked path-form.
